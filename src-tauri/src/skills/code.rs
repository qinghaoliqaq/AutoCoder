...
const SCRATCH_ROOT_DIR: &str = ".ai-dev-hub/subtasks";
...
struct IsolatedWorkspace {
    ...
    base_dir: PathBuf,
    base_snapshot: HashMap<PathBuf, FileFingerprint>,
}

#[derive(Clone, Debug)]
struct WorkspaceChanges {
    changed_or_created: Vec<PathBuf>,
    deleted: Vec<PathBuf>,
}
...
fn sync_coordination_files(main_workspace: &str, isolated_workspace: &Path) -> Result<(), String> {
    for relative in [
        PLAN_MD,
        PLAN_BOARD_MD,
        PLAN_BOARD_JSON,
...
fn cleanup_isolated_workspace(root: &Path) -> Result<(), String> {
    if root.exists() {
...
fn three_way_merge(base: &str, main: &str, ours: &str) -> Result<String, ()> {
    ...
    // Strategy: apply ours_hunks first (they reference base line numbers),
    // then apply main_hunks. Since they don't overlap, order doesn't matter
    // for correctness. We merge by building the result line by line.
...
fn hunks_overlap(a_hunks: &[DiffHunk], b_hunks: &[DiffHunk]) -> bool {
    // For each pair, check if their base ranges overlap.
    // A range [s1, e1) overlaps [s2, e2) iff s1 < e2 && s2 < e1.
    // For pure inserts (base_start == base_end), expand to cover the insertion point.
    for a in a_hunks {
        for b in b_hunks {
...
fn apply_non_overlapping_hunks(
    ...
    // Merge all hunks into one sorted list by base_start.
    ...
    for hunk in &all_hunks {
        // Copy unchanged lines from base up to this hunk.
...
fn snapshot_workspace(root: &Path) -> HashMap<PathBuf, FileFingerprint> {
...
fn relative_paths_from_root(root: &Path, paths: &[PathBuf]) -> Vec<String> {
